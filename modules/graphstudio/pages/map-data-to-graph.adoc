...
GraphStudio supports loading data from local files on the TigerGraph server, from files stored on Amazon S3, or from files stored on Google Cloud Services (GCS).
...
* For *Google Cloud Storage*, skip to the section <<_create_gcs_data_source>>.

== Add data files

A data file is a file containing structured data to be loaded into the graph, creating vertex and/or edge instances.
After specifying your data source, the next step is to specify your data files.
...
After clicking the btn:[ADD] button, your data will appear in the working panel.
...
After you enter your key, enter the URI for your data file in your Google Cloud Storage bucket.

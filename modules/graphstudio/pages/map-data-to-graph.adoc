...
GraphStudio supports loading data from local files on the TigerGraph server or from files stored remotely on Amazon S3, Google Cloud Storage (GCS), or Azure Blob Storage (ABS).
...
== Retrieve files from your data source

If there are data files that are already present on the TigerGraph server (local files), GraphStudio lets you select them directly.
To retrieve files from a remote source such as cloud storage, you must first establish a connection.
...
If you select a folder, all data files in this folder must share the same data schema.

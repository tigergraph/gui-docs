...
To map a column in the data file to a vertex or edge field, drag and drop a row (representing a table in a CSV or a key in a JSON file) from the data file to a target on a vertex or edge.

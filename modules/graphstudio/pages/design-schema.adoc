...
Provide a name for the embedding attribute. The following fields are
...
The *Index type* is set to *HNSW* by default and *Data type* is set
to *FLOAT* by default.

image::embedding_attr_2.png[]

Click the *−* button to the right of an embedding attribute to remove it.

For more information on embedding attribute
fields, see xref:{page-component-version}@gsql-docs:vector:vector-search.adoc[Vector Database Operations].

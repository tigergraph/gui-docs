...
Read more about this architecture, called MultiGraph, at xref:{page-component-version}@tigergraph-server:intro:multigraph-overview.adoc[MultiGraph - An Overview].
...
Under global view, the toolbar area only contains "Save schema", "Add a vertex type", "Add an edge type", "Edit", "Delete", "Undo", and "Redo" buttons.

image::global-view-toolbar-save-schema.png[]
...
Embedding attributes are a special type of attribute that allow you
to store vector embeddings on a vertex. Vector embeddings are
high-dimensional numerical representations of data, such as text,
images, or other unstructured content, generated by AI/ML models.

These attributes enable vector similarity search directly within
the graph database.

To add an embedding attribute, click the *+* button on the right side
of the Embedding Attributes panel:

image::embedding_attr_1.png[]

Provide an `Attribute name` for the embedding attribute. This name is used when referencing the embedding attribute in schema definitions, loading jobs, and vector search operations.
The following fields are
configurable when creating an embedding attribute:

Set the *Dimension* to match the output size of your AI model.
Supported values range from 1 to 4096. For example, a model that produces 128-dimensional vectors should use a dimension of 128.

Choose a *Metric* for the similarity search: `COSINE`,`L2`, or `IP`.

The *Index type* determines how vector similarity searches are performed.
`HNSW` is currently the default and only supported index type.

`FLOAT` is currently the default and only supported *data type*.

image::embedding_attr_2.png[]

To remove an embedding attribute, Click the *−* button to the right of the attribute entry.

For more information on embedding attribute
fields, see xref:{page-component-version}@gsql-ref:vector:vector-search.adoc[Vector Database Operations].
...
Once you are satisfied with the graph schema, click the save schema button image:save-btn.png[image] to save the schema to the TigerGraph system.
If you are publishing a brand new schema, a progress bar will appear:

image:installing_schema_enterprise.png[image]

== Change or edit an existing schema

If a schema already exists, *Save Schema* image:save-btn.png[image] will modify the existing schema.

[WARNING]
====
If you have already loaded data into or created queries for an existing graph, *Save Schema* will only retain your existing data in some circumstances.
====
...
Finally, when you click the save schema
button image:save-btn.png[image], a pop up
...
GraphStudio will try its best to preserve your work when you save your modified schema:
...
.. After the schema is successfully saved, GraphStudio will instruct
you to go to the Map Data To Graph page to verify and save the
revised data mapping. If any mapping is not correct, you can fix it. You *must save* the migrated data mapping; otherwise, it will be lost.

If you have saved some data mapping through GraphStudio, then after the schema is changed successfully, a pop-up window will guide you to go to the Map Data To Graph page to confirm and save the migrated data mapping:

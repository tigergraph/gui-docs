...
image::write-queries-toolbar.png[]
...
* *Console*: Open or close the xref:#_result_panel[Result Panel], which displays query execution results and logs.
...
By default, GraphStudio uses the timeout specified by the `RESTPP.Factory.DefaultQueryTimeoutSec` server parameter. See the xref:{page-component-version}@tigergraph-server:reference:configuration-parameters.adoc#_restpp[TigerGraph Server Command Glossary] for more information.
...
The Profile tab shows a visual flow diagram of each statement in the query.
Each block displays the memory consumed, execution time, and the number of vertices and edges accessed.
The orange bar indicates the relative execution time across statements.

image::profile_tab1.png[]

The *Profile Overview* panel on the right displays a summary of the query execution, including total time, time breakdown by stage, topology access counts, and memory usage.

image:query-profile-visualization.png[image]

For supported query types, how to enable profiling via GSQL or REST API, and current limitations, see xref:gsql-ref:4.2:querying/query-optimizer/query-profiling.adoc[Query Profiling].
...
|View query profile: Display the query execution profile for the most recent query run.

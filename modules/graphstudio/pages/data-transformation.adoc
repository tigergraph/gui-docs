...
. The middle sections shows what the data filter looks like when it is converted a to GSQL WHERE clause. For more details, see the *WHERE Clause* section in the xref:{page-component-version}@gsql-ref:ddl-and-loading:creating-a-loading-job.adoc[].

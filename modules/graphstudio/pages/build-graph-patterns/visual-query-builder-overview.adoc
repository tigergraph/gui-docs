...
Before we introduced *pattern view*, we could only apply filters, attachments, order by and limit on individual vertices or edges. The *pattern view* functionality allows us 
to group vertices and edges within a connected component together as a *pattern view* and apply filters, attachments, order by and limit on the Pattern View itself, instead of 
only on individual vertices and edges. The *pattern view* is applied on the pattern match results and outputs a deduplicated sub-pattern match from the original match.

Our current *pattern view* functionality does not have filter, attachment, order by or limit on it yet, but you can still use it to group vertices and edges in a connected component.
...
As you can see putting a *pattern view* over the whole visual pattern would mean applying the *pattern view* on the pattern match result of the whole pattern. In this case, as 
we do not have any filter, order by or limit the output of the *pattern view* is just the projection the whole pattern match result again.
...
pattern. In this case, it is the sub-graph which matches the sub-pattern (person AS p)-person_has_skill->(skill). As we do not have any filter, order-by and limit the output 
of this *pattern view* is just the projection of the matched sub-graph.
...
result of the whole pattern. In this case, it is all the vertices of type 'person'. As we do not have any filter, order-by and limit, the output of this *pattern view* is just 
the projection of all the distinct vertices of type 'person' within the whole pattern match result. This is the same behavior as when we just output a single vertex. 

...
You can add multiple filters for each vertex and edge pattern, and they are AND relationship when executing the pattern.
Building the filter is similar with building attribute filter in the xref:explore-graph/search-for-vertices.adoc[Explore Graph page].
...
Click image:edit.png[image] beside one attachment and enter editing mode for the attachment.

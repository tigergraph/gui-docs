...
Read more at: xref:{page-component-version}@tigergraph-server:user-access:index.adoc[].

== All Users

The *All Users* tab displays information for all user accounts. The table in this tab includes the following columns:
...
* *Actions*: Provides buttons to change the user's password and delete the user account
...
To see a list of all privileges, see xref:{page-component-version}@tigergraph-server:reference:list-of-privileges-legacy.adoc[List of Privileges].
...
To view all privileges assigned to a specific role, click the role name in the *Role* column.
...
To view all proxy groups, click the *Proxy Group* tab located on the left, under *User*. The table in this tab displays the following columns:
...
Read more about proxy groups at xref:{page-component-version}@tigergraph-server:user-access:index.adoc[].
...
To learn more about roles and privileges in TigerGraph's role-based access control system, see xref:{page-component-version}@tigergraph-server:user-access:index.adoc[].

To create/edit/delete a role, a user must also have the `WRITE_ROLE` privilege on that graph.
See more details in xref:{page-component-version}@tigergraph-server:user-access:access-control-model.adoc[].
...
. Navigate to *User Management* > *User-defined Roles*.
. Create a new role or edit an existing role.
. Under *Local privileges*, select the target graph.
. Expand the graph to display the available vertex and edge types.
. Select the required vertex or edge type.
. Grant the required data privilege (`READ_DATA`, `CREATE_DATA`, or `UPDATE_DATA`).
. Click *Save* to save the role.
. Navigate to *Role Management*.
. Assign the role to the required user.

After the role is assigned, the user can access all attributes of the selected vertex or edge type in GraphStudio.
...
. Create a new role or edit an existing role.
. Under *Local privileges*, select the target graph.
. Expand the required vertex or edge type to display its attributes.
. Select the required attribute or attributes.
. Grant the required data privilege (`READ_DATA`, `CREATE_DATA`, or `UPDATE_DATA`).
. Click *Save* to save the role.
. Navigate to *Role Management*.
. Assign the role to the required user.

After the role is assigned, the user can access only the selected attributes of the vertex or edge type in GraphStudio.

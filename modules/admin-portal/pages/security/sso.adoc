= Single Sign-on (SSO)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;image:enterprise-edition-only.png[Enterprise Edition Only,110]
:description: Admin portal SSO page.
:experimental:

The settings on this page are related to set up Single Sign-on (SSO) on TigerGraph. This page does not appear on the Admin Portal for TigerGraph Cloud solutions because of the unified xref:main@savanna:administration:index.adoc[User/Access Management] system present in TigerGraph Savanna.


Consult the xref:{page-component-version}@tigergraph-server:user-access:sso-with-oidc.adoc[SSO Configuration] page on the TigerGraph Server documentation to read more about the supported SSO methods and the configurations required.
...
[NOTE]
====
In 4.3, SAML SSO supports uploading a full X.509 certificate chain for the Service Provider (SP).

A certificate chain can contain multiple certificates, such as the server certificate, intermediate certificate(s), and root certificate, concatenated in a single file.
...
----
-----BEGIN CERTIFICATE-----
MIID4zCCAc... server cert
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----
MIIE0DCCAr... intermediate cert
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----
MIIFEzCCAv... root cert
-----END CERTIFICATE-----
----
====
...
[NOTE]
====
In 4.3, SAML SSO supports uploading a full X.509 certificate chain for the Identity Provider (IdP).

A certificate chain can contain the identity provider certificate together with any intermediate and root certificates in a single file.
====

...
The settings on this page are related to set up Single Sign-on (SSO) on TigerGraph. This page does not appear on the Admin Portal for TigerGraph Cloud solutions because of the unified xref:cloud:security:manage-org-users.adoc[User Management] system present in TigerGraph Cloud (Classic).

Consult the xref:{page-component-version}@tigergraph-server:user-access:sso-with-oidc.adoc[SSO] page on the TigerGraph Server documentation to read more about the supported SSO methods and the configurations required.

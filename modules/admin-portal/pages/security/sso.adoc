...
[NOTE]
====
When uploading an X509 certificate:

* Including the `-----BEGIN CERTIFICATE-----` and `-----END CERTIFICATE-----` markers is recommended, but not required.
* Empty lines in the certificate file are allowed.
* Certificate files do not need to follow a specific file extension format. If the certificate is displayed correctly after upload, the certificate has been processed successfully.
...
----
-----BEGIN CERTIFICATE-----
MIIEJjCCAw6gAwIBAgICB+YwDQYJKoZIhvcNAQELBQAwgasxCzAJBgNVBAYTAlVT
2LVRSChoH+HDdQ==
-----END CERTIFICATE-----
----
====

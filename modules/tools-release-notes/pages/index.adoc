...
== Sept 2026
=== 2026-09-02
==== GraphStudio Improvements

* Improved graph selection persistence in GraphStudio so the selected graph remains consistent across sidebar navigation, Design Schema, and pages opened from Home in a new tab. Fixed incorrect Global View warnings and session sign-in issues when navigating between pages and tabs.

==== Administration & Security Enhancements

* Improved Role Details for custom roles by displaying configured graph, vertex, edge, and attribute privileges, including `READ_DATA`, `CREATE_DATA`, `UPDATE_DATA`, and `DELETE_DATA`.

==== Insights Improvements

* Improved query-backed dropdown behavior in Insights by displaying a loading indicator while options are loading, applying the default and first selected values correctly, and updating connected visualizations after loading.
* Improved parameter dropdown positioning in Insights Graph widgets so menus remain associated with their fields when scrolling, resizing widgets, or switching queries.
* Improved the Duplicate action in Insights Action Forms by displaying a loading state and disabling the action until duplication completes, preventing duplicate widgets during slow connections. Duplicated forms now retain input values for supported types including `INT`, `STRING`, `BOOL`, `DATETIME`, and `DOUBLE`.k

== Jul 2026
=== 2026-07-06
====  Explore Graph Improvements

* Explore Neighbours now displays only vertex types that are reachable through directed graph edges, making graph traversal more intuitive and reducing confusion.
* Fixed an issue where the graph visualization could disappear after selecting an edge in Explore Graph when working with newly created graphs and expanded vertices.

==== Insights Improvements

* Added support for customizable chart colors, including predefined color palettes, custom series colors, and persistent color settings across supported chart types.
* Improved global variable selection by displaying all available variables with their data types and disabling incompatible options, making parameter selection more transparent and easier to understand.
